// Charger les villes françaises (version client-safe pour Next.js)
// Données statiques des principales villes françaises

// Liste des principales villes françaises avec leurs informations
...
// Charger les villes françaises (version client-safe)
export function loadFrenchCities() {
  return FRENCH_CITIES_DATA;
}

// Fonction pour rechercher une ville par nom (insensible à la casse)
export function findCityByName(cityName) {
  const cities = loadFrenchCities();
  const normalizedCityName = cityName.toLowerCase().replace(/[^a-z]/g, '');
  
  return cities.find(city => {
    const normalizedCity = city.name.toLowerCase().replace(/[^a-z]/g, '');
...
// Fonction pour rechercher des villes par nom (pour l'autocomplétion)
export function searchCities(query) {
  ...
  return cities
    .filter(city => 
      ...
      city.codesPostaux.some(code => code.includes(query))
    )
...
// Exporter la liste complète des villes
export const frenchCities = loadFrenchCities();